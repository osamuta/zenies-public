...
async fn main() {
    ...
    if let (Some(url), Some(target), Some(collection), Some(from), Some(to)) = (
        ...
        if (collection == common_constants::DATABASE_COLLECTION_ORDER_BOOK_BUY
            || collection == common_constants::DATABASE_COLLECTION_ORDER_BOOK_SELL)
            && file_type == "csv"
        {
            eprintln!("{} cannnot be exported as csv file", collection);
            return;
        }
        ...
        let period = match matches.value_of("PERIOD") {
            Some(result) => match result.parse::<f64>() {
                Ok(result) => result,
                Err(result) => {
        ...
        match collection {
            common_constants::DATABASE_COLLECTION_TICKER => {
                            ...
                            match result {
                                Ok(result) => {
                                    if result.timestamp >= latest_time {
                                        latest_time = result.timestamp + period;
                                        data.push(result);
                                    }
                                }
                                Err(result) => {
                ...
                match file_type {
                    ...
                    "json" => {
                        ...
                        match ticker.write(target).await {
                            Ok(result) => result,
                            Err(result) => {
            ...
            common_constants::DATABASE_COLLECTION_EXECUTIONS => {
                ...
                bar.set_style(ProgressStyle::default_bar()
                    .template("Downloading {spinner:.green} [{elapsed_precise}] [{wide_bar:.cyan/blue}] {pos}/{len} ({eta})",).progress_chars("#>-"));
                let mut data = Vec::with_capacity(execution_stream.size_hint().0);
                let mut latest_time = 0.0;
                loop {
                    tokio::select! {
                ...
                match file_type {
                    ...
                    "json" => {
                        let ticker = local::LocalExecutions {
                            collection: String::from(collection),
                            downloaded_at: Utc::now(),
                            start: start_time,
                            end: end_time,
            ...
            | common_constants::DATABASE_COLLECTION_ORDER_BOOK_SELL => {
                ...
                bar.set_style(ProgressStyle::default_bar()
                    .template("Downloading {spinner:.green} [{elapsed_precise}] [{wide_bar:.cyan/blue}] {pos}/{len} ({eta})",).progress_chars("#>-"));
                let mut data = Vec::with_capacity(order_book_stream.size_hint().0);
                let mut latest_time = 0.0;
                loop {
                    tokio::select! {
                        Some(result) = order_book_stream.next() => {
                            ...
                            match result {
                                Ok(result) => {
                                    if result.received_at >= latest_time {
                                        latest_time = result.received_at + period;
                                        data.push(result);
                                    }
                                }
                                Err(result) => {
                ...
                match file_type {
                    ...
                    "json" => {
                        let ticker = local::LocalOrderBook {
                            collection: String::from(collection),
                            downloaded_at: Utc::now(),
                            start: start_time,
                            end: end_time,
